...
const SHELL = `shell-german-${VERSION}`;
const ASSET = `asset-german-${VERSION}`;
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((k) => k.endsWith('-german-' + VERSION) === false && k.includes('-german-'))
  ...
  e.respondWith((async () => {
    try {
      ...
      if (req.mode === 'navigate') {
        const shell = await caches.match('/german/index.html');
