...
const SHELL = `shell-school-${VERSION}`;
const ASSET = `asset-school-${VERSION}`;
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((k) => k.endsWith('-school-' + VERSION) === false && k.includes('-school-'))
  ...
  e.respondWith((async () => {
    try {
      ...
      if (req.mode === 'navigate') {
        const shell = await caches.match('/school/index.html');
