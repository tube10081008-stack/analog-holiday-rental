...
const SHELL = `shell-math-${VERSION}`;
const ASSET = `asset-math-${VERSION}`;
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((k) => k.endsWith('-math-' + VERSION) === false && k.includes('-math-'))
  ...
  e.respondWith((async () => {
    try {
      ...
      if (req.mode === 'navigate') {
        const shell = await caches.match('/math/index.html');
