...
const SHELL = `shell-accounting-${VERSION}`;
const ASSET = `asset-accounting-${VERSION}`;
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys
      .filter((k) => k.endsWith('-accounting-' + VERSION) === false && k.includes('-accounting-'))
  ...
  e.respondWith((async () => {
    try {
      ...
      if (req.mode === 'navigate') {
        const shell = await caches.match('/accounting/index.html');
